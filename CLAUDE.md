...
Single-page React app that finds the optimal layout of Survivor.io "mount" tetromino pieces to maximize a damage-formula score. The user enters their pre-mount percent buffs for 8 stats, marks one or more of the four mounts as **unlocked**, picks one of the unlocked mounts as **equipped**, sets each unlocked mount's star level, and enters their inventory of pieces. The app computes a board layout for every unlocked mount.
...
- **Mounts (4):** `electricScooter` (8×7), `techHoverboard` (8×9), `doomsteed` (8×12), `netherflameStalker` (8×12) — defined in `src/data/mounts.ts` (`MOUNTS` is the single source of truth: name, icon, bg color, board cols, line-bonus tiers, **per-level sync-rate table**). Use `MountKey` and `MOUNTS[key]` everywhere; never hardcode mount metadata. `netherflameStalker` reuses Doomsteed's board width, sync-rate table, and `bgColor`; its line-bonus tiers are distinct and it is the only mount whose no-bonus line is 4 (an explicit `bonus: {}` tier) rather than 2 — the others omit their 2-line tier entirely. An empty-`bonus` tier is inert in `applyLineBonuses` (adds 0), so it behaves like an absent tier for scoring; it only nudges `maxBonusLinesForLevel` up by one at levels 0–1, which is harmless (matches Electric Scooter/Tech Hoverboard's value there anyway).
...
- `usePersistedState` is a `useState`-shaped wrapper around `localStorage`. Versioned keys: `mount-opt:current-stats:v1`, `mount-opt:pieces:v1`, `mount-opt:mode:v1`, `mount-opt:selected-mount:v1`, `mount-opt:mount-levels:v1` (one entry per mount), `mount-opt:unlocked-mounts:v1` (per-mount boolean), `mount-opt:optimize-scope:v1` (`'allUnlocked'` or `'equippedOnly'`), `mount-opt:full-time-limit:v1`, `mount-opt:profiles:v1`. The legacy `mount-opt:mount-level:v1` key is migrated on read into `mountLevels.electricScooter` and otherwise ignored. New users default to **only the previously-selected mount being unlocked** so the legacy single-mount UI is preserved until the user explicitly unlocks more. Pre-Netherflame 3-mount `mount-levels` / `unlocked-mounts` maps are migrated on read (in `legacySeededMountLevels` / `legacySeededUnlocked`) to add `netherflameStalker` (level 0, locked) and the 4-key map is written back so `usePersistedState`'s now-4-key validator accepts it. **Gotcha:** the persisted `unlocked-mounts` value is a **boolean** map, so its migration checks for booleans (`isLegacyBooleanUnlockedMap`) — distinct from the export-string form, which is 1/0 and checked by `isLegacyUnlockedMap`; don't cross the two or existing users' unlocks silently reset.
- `MountPanel` is the 4-up mount picker (lock toggle + icon + per-mount stars + sync-rate readout), laid out `grid-cols-2 sm:grid-cols-4` (2 columns on mobile, 4 on wider screens). The lock toggle on the last-unlocked mount is disabled (≥1 must always be unlocked). Locking the equipped mount auto-switches the equipped to the first remaining unlocked mount (handled in `App.handleUnlockedChange`). Locked mounts can't be selected as equipped (button disabled).
...
- **Export format:** strings are prefixed `mount-opt:vN:`. Current is **v7**, which extends the `u` (unlocked, 1/0) and `lv` (per-mount level) maps to include `netherflameStalker`, on top of v5's `os` (optimize scope: `'a' | 'e'`) and v4's mount key + per-mount levels. v1–v6 importers continue to work — older strings default `netherflameStalker` to **locked** (level **0**); pre-v5 strings additionally default `unlockedMounts` to "selected only" and `optimizeScope` to `'allUnlocked'`. On v5+ import the equipped mount is force-unlocked even if the imported map says otherwise (defensive against hand-edited strings).
